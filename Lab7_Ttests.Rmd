...
and/or a confidence interval. Report the statistical results, and also provide an explanation in 
plain language.

# Data in the wild (again)

For the last part of lab, we are going to integrate today's topic with some real-world messy data. Let's use data from the New York City school voucher lottery a colleage and I obtained from Mathematic for a paper:

```{r eval=FALSE}
nyc = read.csv('input/NYC_SchoolVoucher_Experiment.csv')
```

13. Your first task is to get aquainted with the data. Most of the variables you need should be pretty self explanatory.

14. The second step will of course be to clean the data as needed. 

15. Conduct an analysis to answer the research question: Do eldest children perform better on math than non-eldest children. Be sure to evaluate the validity of the assumptions you need for a t.test to be appropriate (e.g., symmetric distributions, similar sample sizes, etc.). 

16. Now conduct an analysis to see whether the average improvement between year 0 and year 1 is different for "treated" students (those who recieve the voucher) and "untreated students" (those who did not recieve a voucher to attend a private school)

17. Do you have any concerns about these data that might call our findings into question? What else might you want to know more about before you really bought into these results?
