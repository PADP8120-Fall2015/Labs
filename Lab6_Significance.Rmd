...
Lets look at confidence intervals and hypothesis tests using some ‘real’ data. Actually, we can simulate some data and then use R to take samples, compute means and standard deviations, etc. Let’s reconsider a question concerning the amount of soda in some soda bottles. The Coke Bottling Plant in Atlanta produces a huge number of half gallon, plastic Diet Coke bottles. The company claims that, due to manufacturing upgrades, the mean amount of soda
in each bottle they ship has a median volume of 64.00 ounces.

Now suppose we had ALL the data on Coke's Atlanta soda bottles. This is the same as assuming we know everything about the population. We will use R to simulate a population of amounts of soda in 10,000 soda bottles. We assume some distribution function and randomly create 10,000 pieces of data in a variable called pop.

```{r eval=TRUE,echo=FALSE}
pop = 65-rexp(10000,0.65)
hist(pop)
```

Look at the distribution, you’ve produced. It’s definitely not normal (nor should it be, since you drew from an exponential distribution)

4. What does the histogram says about the data. 

5. What are the chances of finding a soda bottle with much less than 64 oz in it? What about much more than 64 oz?

6. Make a boxplot of the data. Is it what you expect? Why?

7. Find the population mean and standard deviation.
...
8. Use a function to preform this same operation in vectorwise fashion (i.e., do this without using a loop!)
...
9. What does the q-q plot tell us? 

10. Change the sample size and compare the qqnorm plots. Try sample sizes n = 64, n = 100 and n = 256.

11. What happens as the sample size decreases? Why?

12. Is the sample mean more Normal for the larger values of n? Show this visually. 
...
13. Use this formula to calculate the SE for your sample mean distribution.
...
14. Use `qt` to find the appropriate critical value (don't forget that a CI is two-sided, so you need to split alpha in half...)

```{r results='hide',echo=FALSE}
alpha = 0.10
zstar = 1-qt(alpha/2,df=50)
```

15. What does this plot represent?

```{r results='hide'}
matplot(rbind(sampmeans2 - zstar*SE, sampmeans2 + zstar*SE),rbind(1:m,1:m), type="l", lty=1)
abline(v=mu)
```

16. How many of these 50 CIs would be expect to not contain the "true" value (shown by the solid black line) simply due to random sampling error? 

17. Change the sample size, n. Try n = 100 and n = 256.

  (a) Replot the 50, 90% confidence intervals. How many of the 50 actually bracket the true mean?

  (b) Set the sample size to n = 40 and change the confidence interval. For $alpha = 0.05$, make a plot of 100 confidence intervals. How many do expect will NOT bracket the true population mean? Is this what you see?
...
18. Pick a different variable from the dataset that we have been using for this section:

```{r eval=FALSE}
worldData = read.csv('input/WDI_GDF_Data.csv', strip.white=TRUE)
```

and perform an analysis like we have done above:

(a) find a raw variable that is not normally distributed

(b) use histograms and transformations to find a method of transformation that better approximates a normal distribution. 