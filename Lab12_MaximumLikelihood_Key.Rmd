...
Yep. It will be the number of columns by the number of rows: 3 x 3

5. Calculate the result.

```{r}
x1 = matrix(c(3,1,2,1,1,0),ncol=2,byrow=T)
x2 = matrix(c(1,0,2,-1,3,1),ncol=3,byrow=T)
x1 %*% x2
```
...
6. Load the Duncan's occupational prestige dataset (`Duncan.txt`) and conduct a matrix-based regression for `income ~ education + prestige`.

```{r}
prestige = read.table('input/Duncan.txt')

#design matrix
x.vars = cbind(1,prestige$education,prestige$prestige)

 # coefficient estimates
betas = solve(t(x.vars) %*% x.vars) %*% t(x.vars) %*% prestige$income

# estimate of sigma-squared
dSigmaSq <- sum((prestige$income - x.vars%*%betas)^2)/(nrow(x.vars)-ncol(x.vars))
...
print(cbind(betas, vStdErr))                            
```

7. Compare your results to the `lm` output version.

```{r}
summary(lm(income ~ education + prestige, data=prestige))
```
...
```{r}
test.vector = c(20,100) #guess values for mu and sigma^2
outp = optim(par = test.vector,fn = NLL, data = x)
outp$par
```
...
```{r}
test.vector = c(10000,10000) #guess values for mu and sigma^2
outp = optim(par = test.vector,fn = NLL, data = x)
outp$par
```

In this case, yes! But, this won't always be the case.
